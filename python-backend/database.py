...
class SupabaseClient:
    ...
    async def get_conference_schedule(
        ...
        """
        try:
            logger.debug(f"Querying conference_schedules with filters: speaker={speaker_name}, topic={topic}, room={conference_room_name}, track={track_name}, date={conference_date}")
            ...
            if conference_date:
                ...
                date_str = conference_date.isoformat() if isinstance(conference_date, date) else str(conference_date)
                query = query.eq("conference_date", date_str)
            if time_range_start:
                query = query.gte("start_time", time_range_start.isoformat())
            if time_range_end:
                query = query.lte("end_time", time_range_end.isoformat())

            response = query.order("conference_date, start_time").execute()
            
            logger.debug(f"Conference schedule query returned {len(response.data) if response.data else 0} results")
